-- ============================================
-- ÉTAPE 2/2 : Configuration du rôle SUPERADMIN (version robuste)
-- ============================================
...
-- Mettre à jour les RLS policies critiques avec gestion des conflits

-- audit_logs : SUPERADMIN peut tout voir
DO $$ 
BEGIN
  DROP POLICY IF EXISTS "Only DG can view audit logs" ON audit_logs;
  DROP POLICY IF EXISTS "SUPERADMIN and DG can view audit logs" ON audit_logs;
EXCEPTION
...
BEGIN
  ...
  DROP POLICY IF EXISTS "SUPERADMIN and admins can manage roles" ON user_roles;
EXCEPTION
  WHEN OTHERS THEN NULL;
END $$;
...
DO $$ 
BEGIN
  DROP POLICY IF EXISTS "SUPERADMIN can manage all profiles" ON ansut_profiles;
EXCEPTION
  WHEN OTHERS THEN NULL;
END $$;
...
DO $$ 
BEGIN
  DROP POLICY IF EXISTS "SUPERADMIN can manage all permissions" ON user_permissions;
EXCEPTION
  WHEN OTHERS THEN NULL;
END $$;
